...
function CheckBadge({ badge, author }: { badge: string; author: User; }): JSX.Element | null {
    ...
    switch (badge) {
        case "EnhancecordDonor":
            return (
                <span style={{ order: settings.store.EnhancecordDonorPosition }}>
                    {badges.getEnhancecordDonorBadges(author.id)?.map((badge: any) => (
                        <RoleIconComponent
        ...
        case "EnhancecordContributer":
            return isEnhancecordPluginDev(author.id) ? (
                <span style={{ order: settings.store.EnhancecordContributorPosition }}>
                    <RoleIconComponent
                        className={roleIconClassName}
...
function ChatBadges({ author }: { author: User; }) {
    ...
    return (
        <span className="vc-sbic-badge-row" style={{ margin: "2px" }}>
            {settings.store.showEnhancecordDonor && <CheckBadge badge={"EnhancecordDonor"} author={author} />}
            {settings.store.showEnhancecordContributor && <CheckBadge badge={"EnhancecordContributer"} author={author} />}
...
export default definePlugin({
    ...
    authors: [Devs.Inbestigator, EnhancecordDevs.KrystalSkull],
